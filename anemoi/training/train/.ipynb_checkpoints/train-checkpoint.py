...
class AnemoiTrainer:
    ...
    def __init__(self, config: DictConfig) -> None:
        ...
        print("*self._update_paths(): ", self._update_paths())
        self._update_paths()
    ...
    def _update_paths(self) -> None:
        ...
        if self.run_id:  # when using mlflow only rank0 will have a run_id except when resuming runs
            ...
            print("*self.config.hardware.paths.checkpoints: ", self.config.hardware.paths.checkpoints)
    ...
    def train(self) -> None:
        ...
        print("*deterministic: ",self.config.training.deterministic)
        print("*detect_anomaly: ",self.config.diagnostics.debug.anomaly_detection)
        print("*strategy: ",self.strategy)
        print("*devices: ",self.config.hardware.num_gpus_per_node)
        print("*num_nodes: ",self.config.hardware.num_nodes)
        print("*precision: ",self.config.training.precision)
        print("*max_epochs: ",self.config.training.max_epochs)
        print("*logger: ",self.loggers)
        print("*log_every_n_steps: ",self.config.diagnostics.log.interval)
        # run a fixed no of batches per epoch (helpful when debugging)
        print("*limit_train_batches: ",self.config.dataloader.limit_batches.training)
        print("*limit_val_batches: ",self.config.dataloader.limit_batches.validation)
        print("*num_sanity_val_steps: ",4)
        print("*accumulate_grad_batches: ",self.config.training.accum_grad_batches)
        print("*gradient_clip_val: ",self.config.training.gradient_clip.val)
        print("*gradient_clip_algorithm: ",self.config.training.gradient_clip.algorithm)
        # we have our own DDP-compliant sampler logic baked into the dataset
        print("*use_distributed_sampler: ",False)
        print("*profiler: ",self.profiler)
        print("*enable_progress_bar: ",self.config.diagnostics.enable_progress_bar)
        ...
        print("*ckpt_path=None if self.load_weights_only else self.last_checkpoint", None if self.load_weights_only else self.last_checkpoint)
        trainer.fit(
            self.model,
